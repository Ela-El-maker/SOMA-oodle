@echo off
REM run-training-thalamus.bat
REM Manually triggers THALAMUS lobe LoRA training against current knowledge library.
REM Produces soma-thalamus:latest in Ollama.
...
echo [SOMA Train] Building THALAMUS training data from knowledge library...
%PYTHON% build-training-data.py --lobe thalamus

REM Find the newest lobe-thalamus-*.jsonl
for /f "delims=" %%i in ('dir /b /od /a-d "SOMA\training-data\lobe-thalamus-*.jsonl" 2^>nul') do set LATEST=%%i
...
set OUTPUT_DIR=models\soma-thalamus-%DATE:~-4,4%%DATE:~-7,2%%DATE:~-10,2%
...
echo   ollama run soma-thalamus:latest
pause
