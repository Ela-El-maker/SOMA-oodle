...
interface Props {
    data: WidgetData;
    allWidgets?: WidgetData[]; // Pass all widgets to read context
}

const InspireWidget: React.FC<Props> = ({ data, allWidgets }) => {
  ...
  const handleInspire = async () => {
    ...
    // Extract context from other widgets
    const identityWidget = allWidgets?.find(w => w.type === 'IDENTITY');
    const textWidget = allWidgets?.find(w => w.type === 'TEXT');
    
    const context = {
        name: identityWidget?.content?.name,
        role: identityWidget?.content?.role,
        bio: identityWidget?.content?.bio,
    ...
    <div className={`w-full h-full flex flex-col relative overflow-hidden group transition-all duration-1000 bg-gradient-to-br ${getGradient(artifact?.type)}`}>
      ...
      <div className="flex items-center justify-between p-6 pb-2 relative z-10">
        <div className="flex items-center gap-2 text-white/60">
            <Zap size={16} className={loading ? 'text-yellow-200 animate-pulse' : 'text-white/60'} />
            <span className="text-xs font-mono uppercase tracking-widest">Creative Engine</span>
        </div>
      </div>
