...
// Converted to ESM to handle mixed module types correctly

import SimulationArbiter from './arbiters/SimulationArbiter.cjs';
import SimulationControllerArbiter from './arbiters/SimulationControllerArbiter.cjs';
...
// DreamArbiter is CJS and exports { DreamArbiter }
import DreamArbiterPkg from './arbiters/DreamArbiter.cjs';
const { DreamArbiter } = DreamArbiterPkg;
...
async function test() {
  ...
  try {
    ...
    const controller = new SimulationControllerArbiter({
      messageBroker: messageBroker,
      autoSaveInterval: 10000, // Save every 10 seconds for diagnostic
      saveAfterEpisodes: 1      // Save after every episode
    });
