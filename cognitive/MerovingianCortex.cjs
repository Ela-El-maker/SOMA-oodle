...
class BrainAdapter {
  ...
  async think(prompt, context = {}) {
    ...
    try {
      ...
      console.error(`${this.name} primary API error:`, error.message);
      // Fallback to local SOMA 1T v1 model
      console.log(`${this.name} attempting fallback to SOMA 1T v1...`);
      try {
        const fallbackResponse = await this.callLlamaFallback(prompt, context);
        console.log(`${this.name} fallback successful`);
        return fallbackResponse;
      } catch (fallbackError) {
        console.error(`${this.name} fallback also failed:`, fallbackError.message);
        return { text: '', confidence: 0.0, reasoning: `All APIs failed: ${error.message} | Fallback: ${fallbackError.message}` };
      }
    }
  }

  async callLlamaFallback(prompt, context) {
    ...
    return new Promise((resolve, reject) => {
      const options = {
        hostname: this.fallbackEndpoint.replace('http://', '').split(':')[0],
        port: parseInt(this.fallbackEndpoint.split(':')[2]) || 11434,
        path: '/api/generate',
        method: 'POST',
      ...
      const req = http.request(options, (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            resolve({ text, confidence, reasoning: 'SOMA 1T v1 fallback (local)' });
          } catch (e) {
            reject(new Error(`Fallback parse error: ${e.message}`));
          }
        });
      });
      req.on('error', reject);
      req.on('timeout', () => reject(new Error('Fallback timeout')));
      req.write(payload);
      req.end();
    });
  }
...
class MerovingianCortex extends BaseArbiter {
  constructor(broker, config = {}) {
    ...
    // Validate API key
    if (!GEMINI_KEY) {
      console.warn('[MerovingianCortex] WARNING: GEMINI_API_KEY not set. Will rely on SOMA 1T v1 fallback only.');
    } else {
      console.log('[MerovingianCortex] GEMINI_API_KEY loaded, fallback to SOMA 1T v1 on error');
    }

    // Initialize QUAD-BRAIN system - ALL using Gemini with fallback to SOMA 1T v1
    this.brains = {
      A: new BrainAdapter(
        'BrainA_Prometheus',
        'You are Prometheus, a strategic and technical AI that plans multi-step solutions and evaluates trade-offs.',
        { type: 'gemini', key: GEMINI_KEY, model: 'gemini-2.0-flash-exp' }
      ),
      B: new BrainAdapter(
        'BrainB_Aurora',
        'You are Aurora, a creative intelligence that explores innovative possibilities and generates novel ideas.',
        { type: 'gemini', key: GEMINI_KEY, model: 'gemini-2.0-flash-exp' }
      ),
      C: new BrainAdapter(
        'BrainC_Logos',
        'You are Logos, an analytical intelligence focused on accuracy, logic, and rigorous reasoning.',
        { type: 'gemini', key: GEMINI_KEY, model: 'gemini-2.0-flash-exp' }
      ),
      D: new BrainAdapter(
        'BrainD_Thalamus',
        'You are Thalamus, the orchestrator that integrates insights from all brains and synthesizes coherent solutions.',
        { type: 'gemini', key: GEMINI_KEY, model: 'gemini-2.0-flash-exp' }
      )
    };
  ...
  async initialize() {
    console.log('🧠 Merovingian: Initializing Quad-Brain Cortex...');
    console.log(`   Brain A (Prometheus): Strategic/Technical`);
    console.log(`   Brain B (Aurora): Creative/Innovative`);
    console.log(`   Brain C (Logos): Analytical/Logical`);
    console.log(`   Brain D (Thalamus): Orchestration/Integration`);

    // Subscribe to query events
    this.broker.subscribe('slc.query', this.handleQuery.bind(this));

    console.log('✅ Merovingian: Quad-Brain Cortex online');
  }
  ...
  async evaluate(query, context = {}) {
    ...
    // PARALLEL QUAD-BRAIN calls (maximum cognitive diversity!)
    this._log('Calling all 4 brains in parallel...');
    const [resA, resB, resC, resD] = await Promise.allSettled([
      ...
      this.brains.D.think(query, { meta, phase: 'initial' })
    ]);
