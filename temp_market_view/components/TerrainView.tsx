...
interface TerrainViewProps {
  ...
  width: number;
  height: number;
  predictions?: PredictionScenario[]; // Multiple projected paths
}

const TerrainView: React.FC<TerrainViewProps> = ({ data, width, height, predictions }) => {
  ...
  useEffect(() => {
    ...
    const minPrice = d3.min(visibleData, d => d.low) || 0;
    const maxPrice = d3.max(visibleData, d => d.high) || 100;
    ...
    const tPct = Math.max(0, Math.min(1, thresholdY / height));
    ...
    // 2. TERRAIN AREA
    const area = d3.area<MarketPoint>()
    ...
    // 3. RIDGE LINE
    const line = d3.line<MarketPoint>()
      .curve(d3.curveMonotoneX)
      .x((_, i) => x(i))
      .y(d => y(d.close));
    ...
    if (predictions && predictions.length > 0) {
      ...
      predictions.forEach(scenario => {
          ...
          const predLine = d3.line<{xVal: number, price: number}>()
              .curve(d3.curveMonotoneX) 
              .x(d => x(d.xVal))
              .y(d => y(d.price));
