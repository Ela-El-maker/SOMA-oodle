...
export const Orb: React.FC<OrbProps> = ({ volume, isActive, isTalking, isListening, isThinking }) => {
  ...
  // Idle breathing animation when not talking
  const breathingAnimation = !isTalking && !isThinking && isActive ? 'animate-pulse' : '';
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div 
          className={`relative w-20 h-20 ${primaryColor} rounded-full blur-md z-30 transition-colors duration-500`}
          style={{ 
            boxShadow: `0 0 80px ${isListening ? 'rgba(96, 165, 250, 1)' : isThinking ? 'rgba(251, 191, 36, 1)' : 'rgba(216, 180, 254, 1)'}`,
          }}
        />

        {/* Layer 2: Inner Glow (State-based color) */}
        <div 
          className={`absolute w-40 h-40 ${secondaryColor} rounded-full blur-xl z-20 mix-blend-screen transition-colors duration-500`}
        />

        {/* Layer 3: Outer Aura (State-based color) */}
        <div 
          className={`absolute w-80 h-80 ${tertiaryColor} rounded-full blur-3xl z-10 opacity-70 mix-blend-screen transition-colors duration-500`}
        />
