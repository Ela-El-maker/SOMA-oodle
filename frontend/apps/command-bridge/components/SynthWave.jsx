...
const SynthWave = ({ volume, isTalking, isActive }) => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // 3 layered sine waves — back to front
      const layers = [
        { freq: 1.8,  phaseOff: 0,          alpha: 0.25, width: 1.2, color: '139,0,255'   },
        { freq: 2.4,  phaseOff: Math.PI / 3, alpha: 0.50, width: 1.8, color: '100,0,200'   },
        { freq: 3.1,  phaseOff: -Math.PI / 5, alpha: 0.90, width: 2.2, color: '168,0,255'  },
      ];
      ...
      // Subtle baseline glow when idle + active
