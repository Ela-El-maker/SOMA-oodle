...
class BaseMicroAgent extends EventEmitter {
  constructor(config = {}) {
    ...
    console.log(`[MicroAgent:${this.id}] Created (type: ${this.type})`);
  }
  ...
  async initialize() {
    ...
    console.log(`[MicroAgent:${this.id}] Initialized`);
  }
  ...
  _startTTLTimer() {
    ...
    this._ttlTimer = setTimeout(() => {
      console.log(`[MicroAgent:${this.id}] TTL expired, terminating`);
  ...
  _resetIdleTimer() {
    ...
    this._idleTimer = setTimeout(() => {
      if (this.state === 'idle') {
        console.log(`[MicroAgent:${this.id}] Idle timeout, terminating`);
  ...
  async executeTask(task) {
    ...
    console.log(`[MicroAgent:${this.id}] Executing task: ${task.type || 'unknown'}`);
    ...
    try {
      ...
      console.log(`[MicroAgent:${this.id}] Task completed in ${executionTime}ms`);
      ...
      console.error(`[MicroAgent:${this.id}] Task failed: ${err.message}`);
      this.emit('task_failed', { id: this.id, error: err });
      
      // Retry logic
      if (this.retryCount < this.maxRetries && task.retryable !== false) {
        this.retryCount++;
        console.log(`[MicroAgent:${this.id}] Retrying (${this.retryCount}/${this.maxRetries})`);
        this.state = 'idle';
  ...
  async terminate(reason = 'manual') {
    ...
    console.log(`[MicroAgent:${this.id}] Terminating (reason: ${reason})`);
